...
use crate::{ColorMapper, ProgressReporter};
    ...
    ($name:ident, $size:literal) => {
        pub fn $name(
            ...
            for _ in 0..$size {
                ...
                let chosen_c = color_map.palette_rgb[chosen];
...
pub fn dither(
    ...
    match matrix_size {
        MatrixSize::Two => image.pixels_mut().enumerate().for_each(|(idx,pixel)| {
            if idx % 1000 == 0 {
                let pct = (idx as f64 / total_pixels as f64) * 100.0f64;
                progress.log_pct("dither", pct).unwrap();
            }
            let (x, y) = (idx % width, idx / width);
            pixel.0 = mix_2x2(pixel.0, multiplier, &color_map, BAYER_2X2[(y as usize % 2) * 2 + (x as usize % 2)]);
        }),
        MatrixSize::Four => image.pixels_mut().enumerate().for_each(|(idx,pixel)| {
            if idx % 1000 == 0 {
                let pct = (idx as f64 / total_pixels as f64) * 100.0f64;
                progress.log_pct("dither", pct).unwrap();
            }
            let (x, y) = (idx % width, idx / width);
            pixel.0 = mix_4x4(pixel.0, multiplier, &color_map, BAYER_4X4[(y as usize % 4) * 4 + (x as usize % 4)]);
        }),
        MatrixSize::Eight => image.pixels_mut().enumerate().for_each(|(idx,pixel)| {
            if idx % 1000 == 0 {
                let pct = (idx as f64 / total_pixels as f64) * 100.0f64;
                progress.log_pct("dither", pct).unwrap();
            }
            let (x, y) = (idx % width, idx / width);
            pixel.0 = mix_8x8(pixel.0, multiplier, &color_map, BAYER_8X8[(y as usize % 8) * 8 + (x as usize % 8)]);
        }),
    };
}
